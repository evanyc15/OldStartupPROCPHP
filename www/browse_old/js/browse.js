$(document).ready(function(){
	...
	var $shove = $('#shove').clone(true,true);
	var tiles = $('#tiles');
	...
	$(window).scroll(function() {
	   ...
	   if($(window).scrollTop() + $(window).height() == $(document).height()) {
	      //pagecount.aftercount = pagecount.aftercount + pagecount.increment;
	      	spinner.show();
			if(pagecount.type == "browse"){
				$lastChild = $('#tiles li:last-child').clone(true,true);
				$.ajax({
					type: 'GET',
					url: 'php/call_db.php',
					dataType: 'json',
						...
						if(response.length != 0){
							for(var i = 0; i < response.length; i++){
								if(response[i] != null && response[i].Sold == 0){
									...
									subject = response[i].Subject;
									//$clone_img = $tile_pic.clone(true,true);
									$clone_img = $lastChild.clone(true,true);
									//$clone_img.fadeTo(0,0);
									$clone_img.find('img').attr({
										src: 'php/image_proxy.php?image='+image
										//alt: url
   ...
   $("#tags").select2({
	...
	$('#tags').on('change',function(){
		...
		var $shove_obj = $shove.clone(true,true);
		tiles.append($shove_obj);
		$('#load-spinner').show();
