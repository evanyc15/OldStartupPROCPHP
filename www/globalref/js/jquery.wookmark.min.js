...
(function (t) {
    ...
    h = function (t, i) {
        ...
        function e(i, e) {
            ...
            for (var o, n = j = 0, r = {}; i.length > n; n++)
                if ($item = i.eq(n), o = $item.data("filterClass"), "object" == typeof o && o.length > 0)
                    for (j = 0; o.length > j; j++) filterClass = t.trim(o[j]).toLowerCase(), filterClass in r || (r[filterClass] = []), r[filterClass].push($item[0]);
        ...
        return e.prototype.update = function (i) {
            ...
            for (var s, h, o, n, r, a = this.placeholders.length, l = this.columns.length, f = this.container.outerHeight(); l > a; a++) s = t('<div class="wookmark-placeholder"/>').appendTo(this.container), this.placeholders.push(s);
            for (innerOffset = this.offset + 2 * parseInt(this.placeholders[0].css("borderWidth")), a = 0; this.placeholders.length > a; a++)
                if (s = this.placeholders[a], o = this.columns[a], a >= l || !o[o.length - 1]) s.css("display", "none");
                else {
                    if (h = o[o.length - 1], !h) continue;
                    r = h.data("wookmark-top") + h.data("wookmark-height") + this.offset, n = f - r - innerOffset, s.css({
                        position: "absolute",
            ...
            if (void 0 === this.itemWidth || 0 === this.itemWidth && !this.flexibleWidth ? t = e.outerWidth() : "string" == typeof this.itemWidth && this.itemWidth.indexOf("%") >= 0 && (t = parseFloat(this.itemWidth) / 100 * i), s) {
                ...
                var h = ~~ (1 + i / (s + this.offset)),
                    o = (i - (h - 1) * this.offset) / h;
                t = Math.max(t, ~~o), this.handler.css("width", t)
            }
            return t
        }, e.prototype.layout = function () {
            if (this.container.is(":visible")) {
                var t, e = this.getItemWidth() + this.offset,
                ...
                if (this.itemHeightsDirty) {
                    for (; r > i; i++) t = n.eq(i), t.data("wookmark-height", t.outerHeight());
                    this.itemHeightsDirty = !1
                }
                h = Math.max(1, Math.min(h, r)), o = "left" == this.align || "right" == this.align ? ~~(h / e + this.offset >> 1) : ~~(.5 + (s - (h * e - this.offset)) >> 1), this.direction = "right" == this.align ? "right" : "left", maxHeight = null != this.columns && this.columns.length == h && this.activeItemCount == r ? this.layoutColumns(e, o) : this.layoutFull(e, h, o), this.activeItemCount = r, this.container.css("height", maxHeight), this.fillEmptySpace && this.refreshPlaceholders(e, o), void 0 !== this.onLayoutChanged && "function" == typeof this.onLayoutChanged && this.onLayoutChanged()
            }
            ...
            var s, h = 0,
                ...
                c = "left" == this.align ? !0 : !1;
            for (this.columns = []; i > u.length;) u.push(0), this.columns.push([]);
            for (; r > h; h++) {
                for ($item = n.eq(h), a = u[0], l = 0, o = 0; i > o; o++) a > u[o] && (a = u[o], l = o);
            ...
            for (var e, s, h, o = [], n = 0, r = 0; this.columns.length > n; n++) {
                for (o.push(0), e = this.columns[n], h = n * t + i, currentHeight = o[n], r = 0; e.length > r; r++) $item = e[r], s = {
                    top: currentHeight
                }, s[this.direction] = h, $item.css(s).data("wookmark-top", currentHeight), currentHeight += $item.data("wookmark-height") + this.offset;
                o[n] = currentHeight
            }
